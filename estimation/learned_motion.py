...
class TorchTcnDisplacementPredictor:
    ...
    def __init__(self, checkpoint_path, *, device: str = "cpu", variance_floor: float = 1.0e-6):
        ...
        if (
            self.target_mode == "kinematic_residual_body_end_gravity_compensated"
            and self.feature_frame != "body_endpoint_gyro_aligned"
        ):
            raise ValueError(
        ...
        if (
            self.target_mode == "delta_velocity_body_end_gyro_aligned"
            and self.feature_frame != "body_endpoint_gyro_aligned"
        ):
            raise ValueError(
                "delta_velocity_body_end_gyro_aligned checkpoints must use "
                "body_endpoint_gyro_aligned features"
            )
